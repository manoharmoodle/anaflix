...
namespace block_custom_courseprogress\external;
...
trait set_block_config {
    ...
    public static function set_block_config($instanceid, $blockhtml, $blockcss, $blockjs) {
        ...
        $returnobj->msg = get_string('changessaved', 'block_custom_courseprogress');

        if (!$blockrecord) {
            $returnobj->status = true;
            $returnobj->msg = get_string('instancenotfound', 'block_custom_courseprogress');
            return $returnobj;
        }
