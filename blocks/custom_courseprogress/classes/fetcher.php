...
namespace block_custom_courseprogress;
...
class fetcher {
     ...
     */
    public function __construct($currentgroup, $timetoshowusers, $context, $sitelevel = true, $courseid = null, $timefrom = null, $timeto = null) {
        $this->set_sql($currentgroup, $timetoshowusers, $context, $sitelevel, $courseid, $timefrom, $timeto);
    }
     ...
     * @param int $courseid The course id to check
     */
    protected function set_sql($currentgroup, $timetoshowusers, $context, $sitelevel, $courseid, $timefrom, $timeto) {
        global $USER, $DB, $CFG;
        if (!$timefrom) {
            $timefrom = 100 * floor(($timeto - $timetoshowusers) / 100); // Round to nearest 100 seconds for better query cache.
        }
        if (!$timeto) {
            $timeto = time();
        }
