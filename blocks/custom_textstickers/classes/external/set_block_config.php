...
namespace block_custom_course_detail\external;
...
trait set_block_config {
    ...
    public static function set_block_config($instanceid, $blockhtml, $blockcss, $blockjs) {
        ...
        $returnobj->msg = get_string('changessaved', 'block_custom_course_detail');

        if (!$blockrecord) {
            $returnobj->status = true;
            $returnobj->msg = get_string('instancenotfound', 'block_custom_course_detail');
            return $returnobj;
        }
