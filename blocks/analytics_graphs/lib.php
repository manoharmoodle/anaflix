...
function block_analytics_graphs_get_resource_url_access($course, $estudantes, $requestedtypes, $startdate, $hidden) {
    ...
    $sqla = "SELECT temp.id+(COALESCE(temp.userid,1)*1000000)as id, temp.timecreated, temp.id as ident, tag.section, m.name as tipo, cm.id as cmid, ";
    $sqlb = "temp.userid, usr.firstname, usr.lastname, usr.email, temp.acessos, tag.sequence
                    FROM (
                        SELECT cm.id, log.userid, log.timecreated, count(*) as acessos
                        FROM {course_modules} cm
