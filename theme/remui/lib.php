...
function theme_remui_get_flat_navigation($flatnavigation) {
    ...
    foreach ($flatnavigation as $navs) {
        ...
        if ($PAGE->pagelayout == 'course' || $PAGE->pagelayout == 'incourse' || $COURSE->id != 1) {
            $cmenus = ['participants', 'badgesview', 'competencies', 'grades', 'calendar', 'contentbank', "coursehome"];

            // adding edwiser plugin support
            $cmenus[] = 'completionreports';

            // adding external plugin support
            $cmenus[] = 'downloadcenter';
        ...
        // YATH Customization :: Code for change admin dashboard link to admin.
        if ($navs->key == "myhome" && is_siteadmin()) {
            $navs->action = $CFG->wwwroot . "/local/admin_dashboard/";

            if ($PAGE->pagelayout == 'mydashboard' || $PAGE->pagetype != 'local-admin_dashboard-index') {
                $navs->isactive = false;
            } else {
                $navs->isactive = true;
            }
