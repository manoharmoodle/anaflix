...
class renderer extends \plugin_renderer_base {
    ...
    public function render_results($results, $page, $totalcount, $url, $cat = null) {
        ...
        if ($results['customdata']) {
            var_dump($results);
            die;
            foreach($results['customdata'] as $custom) {
                $html = '<div class="result">
                <label> '.  .' </label>
                <h4 class="result-title">
                    <img class="icon" alt="" src="https://yislms.com/new_anaflix/moodle/theme/image.php/remui/core/1692862881/i/course"><a href="'.$custom['link'] .'">'.$custom['title'] .'</a>
                </h4>
            </div>';
            $resultshtml[] = $html;
            }
            unset($results['customdata']);
        }
